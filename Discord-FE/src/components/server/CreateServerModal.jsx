...
export default function CreateServerModal({ onClose, onCreated }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      let imageUrl = serverImagePreview;
      if (serverImageFile) {
        const uploadRes = await StorageService.uploadFile(serverImageFile);
        imageUrl = uploadRes.url;
      }*/

      let imageUrl = "https://lh3.googleusercontent.com/a/ACg8ocKmMo19Vt1WHo_oM9THY1GmiP2JzCHh2LAbFy_6ErY0Q8OpAQ=s96-c"; // Placeholder URL
      ...
      const newServer = await ServerChannelService.createServer(userId, payload);
        ...
        {modalType === "main" && (
          <>
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400">{t('Your server is where you and your friends hang out. Make yours and start talking.')}</p>
            </div>
        ...
        {modalType === "create" && (
          <>
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400">{t('Give your new server a personality with a name and an icon. You can always change it later.')}</p>
            </div>
            <div className="flex justify-center mb-4">
              <label className="relative w-24 h-24 rounded-full border-2 border-dashed border-gray-500 flex items-center justify-center cursor-pointer">
                {serverImagePreview ? (
                  <img src={serverImagePreview} alt="Server Icon" className="w-full h-full rounded-full object-cover" />
                ) : (
                  <span className="text-gray-400 text-sm">{t('UPLOAD')}</span>
                )}
                <input type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
                <div className="absolute -bottom-2 right-0 bg-blue-500 w-6 h-6 rounded-full flex items-center justify-center text-white">+</div>
              </label>
            </div>
            {errors.image && <div className="text-red-500 text-sm mb-2">{errors.image}</div>}
            <div className="mb-4">
              ...
              <input
                ...
                value={serverName}
                onChange={(e) => { setServerName(e.target.value); setErrors((prev) => ({ ...prev, name: null })); }}
              />
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className={`bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-md flex items-center transition-colors ${
                  isLoading || !serverName.trim() || !serverImageFile ? "opacity-50 cursor-not-allowed" : ""
                }`}
              >
        ...
        {modalType === "join" && (
          <>
            ...
            <div className="bg-[#2b2d31] p-3 rounded-md flex items-center">
              <div className="bg-green-500 w-6 h-6 rounded-full flex items-center justify-center text-white mr-2">✅</div>
              <span className="text-gray-400 text-sm">{t("Don't have an invite?")}</span>
            </div>
            <div className="flex justify-between mt-4">
              <button onClick={() => setModalType("main")} className="text-gray-400 hover:text-white transition-colors">
                {t('Back')}
              </button>
