...
const VoiceChat = ({ user, channel, onLeave }) => {
  ...
  const [users, setUsers] = useState([
    { _id: currentUser._id, avatar: currentUser.avatar, username: currentUser.username }
  ]);
  ...
  const updateUsers = (newUsers) => {
    ...
    if (!transformed.some((u) => u._id === currentUser._id)) {
      transformed.push({ 
        _id: currentUser._id, 
  ...
  useEffect(() => {
    const join = async () => {
      ...
      const success = await voiceChatService.current.join(currentUser._id, channel.id);
      if (success) {
          ...
          {u._id === currentUser._id && (
            <div className="ml-auto flex items-center gap-2">
              <button onClick={toggleMic}>
                {isMuted ? (
                  <MicOff size={16} className="text-gray-300 hover:text-white" />
                ) : (
